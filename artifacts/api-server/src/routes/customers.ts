...
async function findCustomer(customerId: number): Promise<CustomerRow | null> {
  const [row] = await db
    .select()
    .from(customersTable)
...
router.get("/customers/:customerId", async (req, res): Promise<void> => {
  ...
  const [contacts, addresses, jobRows] = await Promise.all([
    ...
    db
      ...
      .orderBy(customerAddressesTable.createdAt),
    db
      .select()
      .from(jobsTable)
...
router.delete("/customers/:customerId", async (req, res): Promise<void> => {
  ...
  if (linkedJobs.length > 0) {
    res.status(409).json({
      error: `Customer has ${linkedJobs.length} linked job(s). Reassign or delete them first.`,
    });
    return;
  }
...
router.patch("/contacts/:contactId", async (req, res): Promise<void> => {
  ...
  if (body.isPrimary) {
    await db
      .update(contactsTable)
      .set({ isPrimary: false })
      .where(eq(contactsTable.customerId, existing.customerId));
  }
  const [row] = await db
    .update(contactsTable)
    .set(body)
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    const [row] = await db
      ...
      .values({ customerId, label: body.label, address: body.address })
      .returning();
